...
class Html2pdf {
	...
	function create($mode = 'download'){
   		if (is_null($this->html)) {
   			show_error("HTML n'est pas défini");
		}

   		if (is_null($this->path)) {
   			show_error("Le chemin n'est pas défini");
		}

	    

   		if (is_null($this->paper_size)) {
			show_error("Taille du papier non définie");
		}
	    ...
	    if($mode == 'save') {
		    ...
		    if(write_file($this->path.$this->filename, $dompdf->output())) {
		    	...
		    	show_error("Impossible d'écrire le PDF sur le chemin");
		    }
		}
		else {
			...
			show_error("Impossible de diffuser le fichier PDF");
